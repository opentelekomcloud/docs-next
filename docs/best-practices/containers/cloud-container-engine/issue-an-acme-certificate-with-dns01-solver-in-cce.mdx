...
<Tabs>
<TabItem value="existing" label="Use an existing Elastic Load Balancer">
...
- The Load Balancer's Elastic IP is bound to `whoami.example.com`  
- You have replaced the placeholder value `ELB_ID` with the unique identifier of your Elastic Load Balancer  
- You have set the correct value for `kubernetes.io/elb.class`; `union` if is a Shared Load Balancer and `performance` if is a Dedicated one
:::

</TabItem>
<TabItem value="new" label="Automatically create a new Elastic Load Balancer">
...
Before applying the manifest, make sure you have completed the steps outlined in [Automate DNS Records Creation from CCE Ingresses with ExternalDNS](../cloud-container-engine/automate-dns-records-creation-from-cce-ingresses-with-externaldns), as this process will:

* Provision a new dedicated ELB with an associated EIP
* Automatically create the required DNS records in Open Telekom Cloud DNS via ExternalDNS
* Automatically obtain a Let's Encrypt certificate for `whoami.example.com`
