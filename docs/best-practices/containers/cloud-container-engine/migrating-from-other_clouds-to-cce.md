...
<!--     
    ...
    | --provider |                                                              |
    | \--plugins | API component compatible with AWS S3. Both OBS and MinIO     |
    |  | support the S3 protocol.                                     |
    
    | \--b  | Name of the object storage bucket for storing backup files.  |
    | ucket | The bucket must be created in advance.                       |
    +-------+--------------------------------------------------------------+
    | \--s  | Secret file for accessing the object storage, that is, the   |
    | ecret | **credentials-velero** file created in                       |
    | -file | `                                                            |
    |       | 3 <cce_bestpractice_0063__li197871715322>`{.interpreted-text |
    |       | role="ref"}                                                  |
    +-------+--------------------------------------------------------------+
    | \--   | Whether to use Restic to support PV data backup. You are     |
    | use-r | advised to enable this function. Otherwise, storage volume   |
    | estic | resources cannot be backed up.                               |
    +-------+--------------------------------------------------------------+
    | \--   | Whether to create the VolumeSnapshotLocation object for PV   |
    | use-v | snapshot, which requires support from the snapshot program.  |
    | olume | Set this parameter to **false**.                             |
    | -snap |                                                              |
    | shots |                                                              |
    +-------+--------------------------------------------------------------+
    | \--ba | OBS bucket configurations, including region,                 |
    | ckup- | s3ForcePathStyle, and s3Url.                                 |
    | locat |                                                              |
    | ion-c |                                                              |
    | onfig |                                                              |
    +-------+--------------------------------------------------------------+
    | r     | Region to which object storage bucket belongs.               |
    | egion |                                                              |
    |       | -   If OBS is used, set this parameter according to your     |
    |       |     region, for example, **eu-nl**.                          |
    |       | -   If MinIO is used, set this parameter to **minio**.       |
    +-------+--------------------------------------------------------------+
    | s     | The value **true** indicates that the S3 file path format is |
    | 3Forc | used.                                                        |
    | ePath |                                                              |
    | Style |                                                              |
    +-------+--------------------------------------------------------------+
    ...
    |       |                                                              |
    |       | -   If OBS is used, set this parameter to                    |
    |       |     **http://obs.{region}.otc.t-systems.com** (*region*      |
    |       |     indicates the region where the object storage bucket is  |
    |       |     located). For example, if the region is Germany (eu-de), |
    |       |     the value is **http://obs.eu-de.otc.t-systems.com**.     |
    |       |                                                              |
    |       | -   If MinIO is used, set this parameter to **http://{EIP of |
    |       |     the node where minio is located}:9000**. The value of    |
    |       |     this parameter is determined based on the IP address and |
    |       |     port of the node where MinIO is installed.               |
    |       |                                                              |
    |       |     ::: note                                                 |
    |       |     ::: title                                                |
    |       |     Note                                                     |
    |       |     :::                                                      |
    |       |                                                              |
    |       |     -   The access port in s3Url must be set to the API port |
    |       |         of MinIO instead of the console port. The default    |
    |       |         API port of MinIO is 9000.                           |
    |       |     -   To access MinIO installed outside the cluster, enter |
    |       |         the public IP address of MinIO.                      |
    |       |     :::                                                      |
    +-------+--------------------------------------------------------------+
