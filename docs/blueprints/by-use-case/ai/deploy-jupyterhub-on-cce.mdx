...
This article explains how to deploy JupyterHub on Cloud Container Engine (CCE) as a multi-user service on T Cloud Public. It focuses on the platform and architectural considerations required to run JupyterHub reliably on Kubernetes, including integration with T Cloud Public services and commonly used components from the open source ecosystem.
...
[Enabling External Traffic with Ingress & TLS](/docs/best-practices/containers/cloud-container-engine/enabling-external-traffic-with-ingress-api).
