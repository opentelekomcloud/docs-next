...
title: Configure Dokploy for T Cloud Public
...
:::warning attention
All of the following steps must be configured for Dokploy to function correctly on T Cloud Public.
:::

## Configuring Registries

To use Dokploy effectively on T Cloud Public, you must connect it to a container registry. In the registry is where application images will going to be stored, versioned, and retrieved during deployments. Dokploy needs this integration to push newly built images and to pull the correct versions when running applications. On T Cloud Public, the Software Repository for Containers (SWR) is typically used, but external registries such as Docker Hub or GitHub Container Registry can also be configured, although SWR is highly recommended for tighter integration with the rest of the T Cloud Public ecosystem and managed services. This step ensures that Dokploy can access images securely and deliver applications in a consistent and automated way.
