...
Create a file named `traefik.yml` in the `/mnt/sfs_turbo` directory: 

```yml
# Traefik v2.10 static configuration
# - Only HTTP on :80 (TLS is terminated at the ELB)
# - Docker Swarm provider
# - Trust ELB's forwarded headers (includes CGNAT 100.64.0.0/10)
# - Health endpoint at /ping for ELB checks
...
Create a file named `portainer-stack.yml` in the `/mnt/sfs_turbo` directory:
