...
Go to *T Cloud Public Console* -> *Network Console* -> *Virtual Private Cloud* and click *Create VPC*. Create a VPC (let's call it for the rest of this lab `vpc-dokploy`) with the IPv4 CIDR block `10.199.0.0/16`, and define three subnets:
...
1. Go to *T Cloud Public Console* -> *Relational Database Service* and click **Create DB Instance**. Name the instance `rds-dokploy` and configure it with the following settings:
...
Go to *T Cloud Public Console* -> *Distributed Cache Service* and click *Create DCS Instance*. Name the instance `dcs-dokploy` and configure it with the following settings:
...
Go to *T Cloud Public Console* -> *Network Console* -> *Access Control* -> *Security Groups* and click *Create Security Group*:
...
Go to *T Cloud Public Console* -> *Elastic Cloud Server* -> *ECS Group*  and click *Create ECS Group*. 
...
Go to *T Cloud Public Console* -> *Elastic Cloud Server* -> *Network & Security* -> *Key Pair*  and click *Create Key Pair*. 
...
<Tabs>
    <TabItem value="masters" label="Master Nodes" default>
        Go to *T Cloud Public Console* -> *Elastic Cloud Server*  and click *Create ECS* and configure it with the following settings:
...
Go to *T Cloud Public Console* -> *Scalable File System* -> *SFS Turbo* -> *File Systems* and click *Create File System*. 
...
Go to *T Cloud Public Console* -> *Object Storage Service* -> *Object Storage* and click *Create Bucket*. 
...
cloud deployments the same way. By giving it an OBS bucket, you satisfy that contract with an API that looks like AWS S3 but lives entirely in T Cloud Public, 
...
Go to *T Cloud Public Console* -> *Network Console* -> *NAT Gateway* -> *Public NAT Gateways* and click *Create Public NAT Gateway*. Call it `nat-dokploy` and assign 
...
1. Go to *T Cloud Public Console* -> *Network Console* -> *Elastic IP & Bandwidth* -> *EIPs* and click *Assign EIP*. Name it `eip-natgw-dokploy` and configure 
...
2. Go to *T Cloud Public Console* -> *Network Console* -> *NAT Gateway* -> *Public NAT Gateways* and select the NAT gateway you created earlier.
...
Go to *T Cloud Public Console* -> *Network Console* -> *Elastic Load Balancing* and click *Create Elastic Load Balancer*. Name the instance `elb-dokploy` 
...
:::caution
You must delegate the management of your domain's NS records to the T Cloud Public Domain Name Service.
...
Once these changes are in place, you can proceed to create a new DNS Public Zone and the required record sets for your domain in T Cloud Public.

If delegating domain management to T Cloud Public DNS **is not an option**, 
...
Go to *T Cloud Public Console* -> *Domain Name Service* -> *Public Zones* and select the zone corresponding 
...
:::important
Replace `10.199.0.0/16` with the CIDR block of your VPC.
:::
...
:::note TLS Termination & ELB Health Checks
...
reliably share ACME certificate state across replicas, TLS termination is handled at the **T Cloud Public Elastic Load Balancer (ELB)** instead. 
...
Access to Traefik is restricted so that it only accepts requests originating from the VPC subnets and from T Cloud Public infrastructure 
...
the ELB listeners](#creating-elb-listeners).
:::
...
* `--manual`, instructs Certbot not to use an automated DNS or HTTP plugin, but instead to guide you through the challenge process manually. In this mode it will stop and display TXT records that you must create in your domain’s DNS zone.
...
T Cloud Public and use in your ELB to terminate TLS for the entire platform.
:::

Go to *T Cloud Public Console* -> *Network Console* -> *Elastic Load Balancing* -> *Certificates* and click *Add Certificate*. Name it `cert-wildcard-dokploy`
...
Go to *T Cloud Public Console* -> *Network Console* -> *Elastic Load Balancing* -> *Load Balancers*, select the ELB we created for this lab and click *Add Listener*. 
...
Go to *T Cloud Public Console* -> *Relational Database Service*. Choose your database, go to *Plugins*, 
