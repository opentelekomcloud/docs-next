...
:::note
If you don't have a server-side encryption key, you can give an empty string for `sse_c_key` value.
:::
...
:::info
You find Host and Port of the RDS under the **Basic Information** page of your RDS instance under **Floating IP Address** and **Database Port** fields.
:::
...
The `image.flavor` field determines which image flavor of Nextcloud you want to use. We use the `fpm` for our example as it is better for HA setups.
...
2. **Annotations Explanation:**
   ...
   You may need to restart **nginx-ingress-controller** pod after making these changes for them to take effect.
...
nextcloud:
# ... other fields ...
# `objectStore` is subfield of the `nextcloud` field.
  objectStore:
      s3:
        enabled: true
        legacyAuth: false
...
Here’s an explanation of the fields:
   ...
   - `storageClass: STANDARD` selects the storage class, **STANDARD** is typical while using S3 compatible Object Storage.
...
nextcloud:
# ... other fields ...
# `defaultConfigs` is subfield of the `nextcloud` field.
  defaultConfigs:
...
:::info
Imaginary is a service that enhances Nextcloud ability to process and manage images efficiently. Visit [Imaginary](https://github.com/h2non/imaginary) for more information.
:::
...
nextcloud:
# ... other fields ...
# `configs` is subfield of the `nextcloud` field.
  configs:
...
nextcloud:
# ... other fields ...
# `strategy` is subfield of the `nextcloud` field.
strategy:
  type: RollingUpdate
  rollingUpdate:
    maxSurge: 25%
...
### Cronjob
The `cronjob` field specifies the configuration to use a cron sidecar container in Nextcloud pod to run cron jobs in the background. Background jobs are important for tasks that do not necessarily need user intervention, but still need to be executed frequently (cleaning up, sending some notifications, pulling RSS feeds, etc.)
...
helm repo update
```
2. **Prepare values**: Incorporate the sections above, filling in your domain, secrets names, storageClass, endpoints, etc. Here is an example of a `values.yaml`:
```yaml
image:
  flavor: fpm
...
2. **Install/Upgrade:** Run following command to install/upgrade Nextcloud:
...
3. **Verify Pods & Services:** Run these commands to ensure Nextcloud is ready:
...
* Login with the credential provided in the **nexcloud** secret . Complete admin account setup like instating preferred apps, etc.
...
## Validation & Testing
You can check the security of your setup with the [Nextcloud Security Scan](https://scan.nextcloud.com/). If everything is secure, you will get a **A+** score. If not you can check the scan results for specific issues.
...
* **File Upload/Download**: Upload files of various sizes and observe that files end up encrypted in OBS bucket .
